        ...
        <div className="max-w-2xl mx-auto space-y-12 pb-20">
          ...
          >
            <div className="space-y-3">
              <h3 className="text-2xl font-urbanist font-bold text-slate-900 leading-tight">Read carefully before submitting</h3>
              <p className="text-slate-500 text-sm font-inter leading-relaxed">
                Veridex's Preparation Track is designed for students who are willing to commit time and effort to becoming task-ready.<br />
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-2">
                <Label className="text-sm font-urbanist font-bold text-slate-900">What is your primary goal after completing the preparation track?</Label>
                <Select 
              ...
              <div className="space-y-4 pt-4">
                ...
                <div className="space-y-3">
                  <label className="flex items-start gap-3 p-5 rounded-xl border border-slate-100 bg-white hover:border-slate-200 transition-all cursor-pointer">
                    <input 
                      type="checkbox" 
                    ...
                    />
                    <span className="text-sm font-inter text-slate-600">I understand this does not guarantee acceptance into Veridex Core</span>
                  </label>
                  ...
                  <label className="flex items-start gap-3 p-5 rounded-xl border border-slate-100 bg-white hover:border-slate-200 transition-all cursor-pointer">
                    <input 
                      ...
                      className="mt-1 w-5 h-5 rounded border-slate-300 text-slate-900 focus:ring-slate-900"
                    />
                    <span className="text-sm font-inter text-slate-600">I am willing to put in consistent effort to improve my skills</span>
                  </label>
                </div>
              </div>
            </div>
    ...
    <ApplicationLayout currentStep={3} totalSteps={3} title="Operational Capacity" onBack={onBack}>
      <div className="max-w-2xl mx-auto space-y-12 pb-20">
        <div className="text-center space-y-3">
          <h2 className="text-2xl font-urbanist font-bold text-slate-900">Delivery Commitment</h2>
        ...
        <div className="grid grid-cols-1 gap-4">
          {commitmentCards.map((card) => (
            <button
              ...
              className={`group relative flex items-start gap-6 p-6 rounded-2xl border transition-all text-left cursor-pointer ${selectedCommitment === card.id ? 'border-slate-900 bg-white shadow-xl shadow-slate-200/50' : 'border-slate-100 bg-slate-50/30 hover:border-slate-200 hover:bg-white'}`}
            >
              <div className={`p-3 rounded-xl ${selectedCommitment === card.id ? 'bg-slate-900 text-white' : 'bg-white text-slate-400 border border-slate-100 shadow-sm'}`}>
                <card.icon className="w-6 h-6" />
              </div>
              <div className="flex-1 space-y-2">
                <span className={`text-[10px] font-mono font-bold tracking-widest px-2 py-0.5 rounded uppercase ${card.statusColor}`}>{card.status}</span>
                <h3 className="text-lg font-urbanist font-bold text-slate-900">{card.title}</h3>
                <p className="text-sm text-slate-500 font-inter leading-relaxed">{card.desc}</p>
              </div>
            </button>
          ))}
        </div>
        ...
        <AnimatePresence mode="wait">
          {selectedCommitment && selectedCommitment !== 'none' && (
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-8">
              <div className="p-8 bg-slate-900 rounded-2xl text-white shadow-2xl shadow-indigo-500/10">
                <div className="flex items-center gap-3 mb-6">
                  <FileText className="w-5 h-5 text-indigo-400" />
                  <span className="text-[10px] font-mono font-bold tracking-widest text-indigo-400 uppercase">PROTOCOL: DELIVERY_COMMITMENT_STMT</span>
                </div>
                <p className="text-sm font-inter text-slate-300 leading-relaxed mb-8">
                ...
                <label className="flex items-start gap-3 p-4 rounded-xl bg-white/5 border border-white/10 hover:bg-white/10 transition-colors cursor-pointer group">
                  <input 
                    type="checkbox" 
                    checked={confirmedStatus.coreConfirmed}
                    onChange={(e) => setConfirmedStatus({...confirmedStatus, coreConfirmed: e.target.checked})}
                    className="w-4 h-4 rounded border-slate-700 bg-transparent text-indigo-500 focus:ring-indigo-500"
                  />
                  <span className="text-xs font-urbanist font-bold text-white uppercase tracking-tight">Confirm {selectedCommitment === 'core' ? '8–12' : '4–7'} hour weekly capacity</span>
                </label>
              </div>
              <Button 
                onClick={handleFinalize}
                disabled={!confirmedStatus.coreConfirmed}
                className={`w-full h-14 rounded-xl shadow-xl font-urbanist font-bold transition-all ${confirmedStatus.coreConfirmed ? 'bg-slate-900 hover:bg-black text-white shadow-slate-200' : 'bg-slate-100 text-slate-300 shadow-none'}`}
              >
