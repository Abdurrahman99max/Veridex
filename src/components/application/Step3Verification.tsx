...
export const Step3Verification: React.FC<VerificationProps> = ({ onNext, onBack, prevData, track }) => {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (selected) {
      if (selected.size > 10 * 1024 * 1024) {
        setErrorMessage('Document exceeds 10MB limit.');
        return;
      }
      setFile(selected);
      setErrorMessage('');
  ...
  const startVerification = async () => {
    ...
    try {
      ...
      onNext({ documentPath: filePath, documentType });
    } catch (err: any) {
            ...
            {status === 'idle' || status === 'error' ? (
              ...
              >
                ...
                <div className="space-y-3">
                  <Label htmlFor="document-type" className="text-sm font-urbanist font-bold text-slate-900">
                    Document Type
                  </Label>
                  <Select value={documentType} onValueChange={setDocumentType}>
                    <SelectTrigger className="w-full h-14 rounded-xl border-slate-200 bg-white">
                      <SelectValue placeholder="Select the type of document you're uploading" />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value="school_id">School ID Card</SelectItem>
                      <SelectItem value="portal_screenshot">Student Portal Screenshot</SelectItem>
                      <SelectItem value="registration_document">Registration/Enrollment Document</SelectItem>
                      <SelectItem value="acceptance_letter">Acceptance Letter</SelectItem>
                      <SelectItem value="current_schedule">Current Semester Schedule</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
                ...
                >
                  ...
                  {file ? (
                    ...
                    <div className="text-center space-y-3">
                      ...
                      <div>
                        <p className="text-sm font-urbanist font-bold text-slate-900">Upload Verification Document</p>
                        <p className="text-xs text-slate-400 font-inter mt-1">Max 10MB</p>
                      </div>
                    </div>
                  )}
                </div>
                ...
                <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
                  <p className="text-xs font-urbanist font-bold text-slate-900 mb-2">Accepted File Types:</p>
                  <div className="flex flex-wrap gap-2">
                    <span className="text-[10px] font-mono font-bold text-slate-500 bg-white px-2 py-1 rounded border border-slate-200">PDF</span>
                    <span className="text-[10px] font-mono font-bold text-slate-500 bg-white px-2 py-1 rounded border border-slate-200">DOC/DOCX</span>
                    <span className="text-[10px] font-mono font-bold text-slate-500 bg-white px-2 py-1 rounded border border-slate-200">JPG/PNG</span>
                  </div>
                </div>
                ...
                <Button 
                  ...
                  className={`w-full h-14 rounded-xl shadow-xl font-urbanist font-bold transition-all text-sm sm:text-base ${file && documentType && !isUploading ? 'bg-slate-900 hover:bg-black text-white' : 'bg-slate-100 text-slate-300'}`}
                >
                  Verify Identity
