...
// Helper: Initialize admin whitelist with master admin
const initializeAdminWhitelist = async (): Promise<AdminUser[]> => {
  const existing = await kv.get('admin_whitelist');
  
  // If already initialized with role-based structure, return it
  if (existing && Array.isArray(existing) && existing.length > 0 && typeof existing[0] === 'object' && 'role' in existing[0]) {
    return existing as AdminUser[];
  }
  
  // If it's old string array format, migrate it
  if (existing && Array.isArray(existing) && existing.length > 0 && typeof existing[0] === 'string') {
    const migrated: AdminUser[] = existing.map((email: string) => ({
      email,
      role: (email === MASTER_ADMIN ? 'super_admin' : 'admin') as AdminRole,
      addedBy: 'SYSTEM',
      addedAt: new Date().toISOString()
    }));
    await kv.set('admin_whitelist', migrated);
    return migrated;
  }
  ...
  await kv.set('admin_whitelist', initial);
  return initial;
};

// Helper: Check if email is in whitelist (works with both old and new format)
const isWhitelisted = (whitelist: (string | AdminUser)[], email: string): boolean => {
...
const getAdminRole = (whitelist: AdminUser[], email: string): AdminRole | null => {
  const admin = whitelist.find(a => a.email.toLowerCase() === email.toLowerCase());
...
app.post(`${prefix}/admin/request-otp`, async (c) => {
  try {
    const { email } = await c.req.json();
    const whitelist = await initializeAdminWhitelist();
    
    if (!isWhitelisted(whitelist, email)) {
...
app.post(`${prefix}/admin/whitelist`, async (c) => {
  try {
    const { email, role, action, addedBy, name } = await c.req.json();
    const current = await initializeAdminWhitelist();
    
    if (action === 'add') {
      // Check if already exists
      const exists = current.find(a => a.email.toLowerCase() === email.toLowerCase());
      if (exists) {
        return c.json({ error: 'Admin already exists' }, 400);
      }
      
      // Prevent adding another super admin
      if (role === 'super_admin' && email.toLowerCase() !== MASTER_ADMIN) {
        return c.json({ error: 'Only one super admin is allowed' }, 400);
      }
